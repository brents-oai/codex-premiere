...
function readRenameClipInstancesPayload(args) {
  const payload = {};

  if (args.name === undefined) {
    throw new Error("Provide --name for rename-clip-instances");
  }
  const desiredName = String(args.name);
  if (!desiredName.trim()) {
    throw new Error("--name must be a non-empty string");
  }
...
function readReplaceClipSourcePayload(args) {
  ...
  if (args["item-id"] === undefined || args["item-id"] === null || String(args["item-id"]).trim() === "") {
    throw new Error("Provide --item-id for replace-clip-source");
  }
  payload.itemId = String(args["item-id"]);
...
async function main() {
  ...
  if (command === "overwrite-clip") {
    if ((config.transport || "").toLowerCase() === "uxp") {
      throw new Error("overwrite-clip is currently supported only on CEP. Use --transport cep.");
    }
    if (args["item-id"] === undefined || args["item-id"] === null || String(args["item-id"]).trim() === "") {
      throw new Error("Provide --item-id for overwrite-clip");
    }
    ...
    if (locationKeys.length !== 1) {
      throw new Error("Provide exactly one overwrite location: --at playhead, --timecode, --seconds, or --ticks");
    }
    ...
    if (args.at !== undefined) {
      ...
      if (!playheadResult.body || playheadResult.body.ok !== true || !playheadResult.body.data) {
        throw new Error(
          `Failed to resolve playhead for overwrite-clip: ${playheadResult.body && playheadResult.body.error}`
        );
      }
      payload.ticks = Number(playheadResult.body.data.ticks);
      if (playheadResult.body.data.timecode) {
    ...
    const result = await sendCommandCep(config, "overwriteClip", attachDryRun(payload, dryRun));
    console.log(JSON.stringify(result, null, 2));
    return;
  }

  if (command === "rename-clip-instances") {
    if ((config.transport || "").toLowerCase() === "uxp") {
      throw new Error("rename-clip-instances is currently supported only on CEP. Use --transport cep.");
    }
  ...
  if (command === "replace-clip-source") {
    if ((config.transport || "").toLowerCase() === "uxp") {
      throw new Error("replace-clip-source is currently supported only on CEP. Use --transport cep.");
    }
    const payload = readReplaceClipSourcePayload(args);
    const result = await sendCommandCep(config, "replaceClipSource", attachDryRun(payload, dryRun));
    console.log(JSON.stringify(result, null, 2));
    return;
  }
